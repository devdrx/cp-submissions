...
void solve(){
    ...
    //check if the whole string is made up of only character c
    bool allc=1;
    for(auto x:s){
        if(x!=c)   allc=0;
    }
    if(!allc){
        //go through all divisors of string size
        for(int i = 1; i < n+1; i++){
            int f = 1;
            for(int j = i; j < n+1; j++){
                f = f & (s[j-1] == c);
            ...
            //if found any divisor which has c in every multiple of i then 1 operation is sufficient
            if(f){
                cout << 1 << endl;
                cout << i << endl;
                return;
