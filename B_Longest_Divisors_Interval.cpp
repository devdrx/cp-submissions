...
// Function to calculate all the divisors of a given number
...
// Function to find the longest sequence of adjacent numbers in a vector
int longestAdjacentSequence(const std::vector<int>& nums) {
    int longestSeq = 1; // Initialize with at least one element
    int currentSeq = 1; // Length of current sequence
    
    for (size_t i = 1; i < nums.size(); ++i) {
        if (nums[i] == nums[i - 1] + 1) {
            currentSeq++; // Increment current sequence length if adjacent
            longestSeq = std::max(longestSeq, currentSeq);
        } else {
            currentSeq = 1; // Reset current sequence length
        }
    }
