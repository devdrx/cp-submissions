...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        int k = i, zeros = 0, ones = 0, f = -1; // f is the first non zero element index
        while(k < n){
            if(a[k] != 0){
                ones++;
                if(f == -1){
        ...
        zeros-=2;
        if(k>=n){
            b.push_back({i+1,n}); //trailing zeros at the end
        }
        if(f-i > 0 and f!=-1){
            b.push_back({i+1,f}); //leading zeros at the beginning
        } 
        if(ones == 2){ //normal case
            if(a[f]+a[k] == 0){
                b.push_back({f+1,k});
                b.push_back({k+1,k+1});
            }
