...
void solve(){
    ...
    //for every index we stored the index where there is a different number
    int q; cin >> q;
    while(q--){
        int l,r ; cin >> l >> r;
        l--; r--;
        if(ans[l]<=r){
            //if for L we have its answer at some index less than R, then we can directly print the answer
