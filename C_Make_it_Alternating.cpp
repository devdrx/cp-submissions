...
void solve(){
    ...
    for(int i = 0; i < n-1; i++){
        if(s[i]==s[i+1]){
            //extra character in consecutive sequence
            seqn++;
        }
        else{
            ans1 += seqn;
            seqn++; //for multiplying ways seqn+1 C seqn = seqn+1 //u can choose seqn index from a seqn + 1 length same character sequence
            ans2 = modmul(ans2, seqn);
            seqn = 0;
        }
    }
    if(seqn!=0){ //because the for loop doesnt cover it
        ans1+=seqn;
        seqn++;
        ans2 = modmul(ans2, seqn);
    }
    int fack = 1; //ans1 is the no of indices u are taking to remove, i am calculating ans1!
    for(int i = 1; i <= ans1; i++){
        fack = modmul(fack, i);
    }
    ans2 = modmul(ans2, fack); //multiplying ans1! with ans2 (the no of permutations of all the indices)
    cout << ans1 << " " << ans2; nl;
}
