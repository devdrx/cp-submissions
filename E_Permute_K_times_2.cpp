...
void solve(){
    int n, k;
    cin >> n >> k;
    vector<int> permutation(n); cin >> permutation;
    vector<int> sequence, permuation;
    for(int i = 0; i<=n; i++){
        sequence.push_back(i);
    }
    permuation.push_back(0);
    for(int i = 1; i <= n; i++){
        permuation.push_back(permutation[i-1]);
    }
    sequence = permute(sequence, permuation, k);
