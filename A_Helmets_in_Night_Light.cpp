...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        a[i].first = min(a[i].first, k); //if its better for chief to send info himself then let him cook
    }
    sort(a.begin(), a.end()); //sorted by increasing costs
    int ans = k; int rem = n-1;
    for(int i = 0; i < n-1&&rem; i++){
        ans += min(a[i].second, rem)*a[i].first;    //no of people times the cost of information 
        rem -= min(a[i].second, rem);  //deducting from remaining
    }
    cout << ans << endl;
}\
