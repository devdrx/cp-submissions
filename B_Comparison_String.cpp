...
// ==================================== MATH UTIL ENDS=======================================================//
int longestSingleCharacterSubsequence(const std::string& str) {
    if (str.empty()) {
    ...
    for (size_t i = 1; i < str.size(); ++i) {
        if (str[i] == prevChar) {
            ...
            prevChar = str[i]; // Update previous character
            maxCount = std::max(maxCount, count); // Update maxCount if necessary
            count = 1; // Reset count for new character
        }
    }

    maxCount = std::max(maxCount, count); // Update maxCount after the loop
...
void solve(){
    ...
    cin>>n;
    cin>>s;
    int ans = longestSingleCharacterSubsequence(s);
    cout << ans + 1 << endl;
}
