...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        ...
        if(one == zero){
            locations.push_back({left,i});
            left = i+1;
        }
    }
    //now for each of these balanced segments
    for(auto seg: locations){
        int start = seg.first;
        int end = seg.second;
        //matching these segments
        //if segment need not be flipped then leave after checking first character
