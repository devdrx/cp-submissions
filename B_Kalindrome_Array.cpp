...
// ==================================== MATH UTIL ENDS=======================================================//
//function to check if palindrome given removal of number x from array
...
void solve(){
    ...
    vi v(n);
    cin>>v;
    //if deleting some of the same elements of the array can make it a palindrome, then deleting all of them will also make it 
    //a palindrome (by symmetry) 
    int left = 1; //we are checking by invalidating the vector, so first it should be true
    int right = 1;
    for(int i = 0; i < n; i++){
        if(v[i] != v[n-1-i]){ //first occurence of palindrome not being formed
            left = check(v, v[i]); //inn dono mein so koi ek ka hin removal sufficient hona chahiye, nhin to palindrom hin nhin banega
            right = check(v, v[n-1-i]);
            break;
        }
    }
    if(left or right){
