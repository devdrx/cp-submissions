...
void solve(){
    ...
    if(st.size()==1){cout<<"NO"<<endl;}
        ...
        vector<vi> other;
        for(auto i : a){
            other.push_back(i.second); //pushback all vectors individually
        }
        for(int i = 1; i<other.size(); i++){
            for(auto j : other[i]){
                cout << other[0][0]+1<<" "<<j+1<<endl; //connect first gang to all the other gangs
            }
        }
        for(int i = 1; i < other[0].size(); i++){
            cout << other[0][i]+1 << " " << other[1][0]+1 <<endl;
            //connect rest of the first gangs to the first place of the second gang
        }
    }
}
