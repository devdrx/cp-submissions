...
void solve(){
    ...
    int ans = INT_MIN;
    for(int i = 1; i<=n; i++){
        //start at position where prefix sum is greater than sum+pref[i-1] as right most
        auto it = lower_bound(all(pref),sum+pref[i-1]);
        if(it!=pref.end()){
            int loc = it - pref.begin();
            int val = pref[loc];
            //to add zeros to the right
            while(pref[loc+1]==val){
                loc++;
                if(loc==n+1){
                    loc--; break;
                }
