...
void solve()
{
    ...
    while (q--)
    {
        ...
        while (m--)
        {
            ...
            if (ch == '>')
            {
                int lo = upper_bound(b.begin(), b.end(), c, [&](int val, const vector<int>& row) { return val < row[r]; }) - b.begin();
            ...
            {
                //find last
                int i = lower_bound(b.begin(), b.end(), c, [&](const vector<int>& row, int val) { return row[r] < val; }) - b.begin();
                R = min(R, i-1);
            }
        }
