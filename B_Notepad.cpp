...
void solve(){
    ...
    for(int i=0;i<n-1;i++){
        ...
        if(mp.find(temp)!=mp.end()){ //if found
            //check if the index of the found substring is not adjacent to the current substring
            if(mp[temp]==i-1){
                continue;
            }
            else{
            ...
            // mp[temp] = i; // update the index of the substring
        }
        mp[temp] = i; // store the index of the substring
        // for(auto x: mp){
