...
void solve(){
    ...
    while(ida < n && idb < n){
        if(a[ida] >= b[idb]){
            idb++; //we dont want this
        }
        else{
            idb++; // we want this to happen
            ida++;
        }
    }//either min element of a is greater than max of b, in which case we are left with nothing or we have some array of k size that can dance jingalala
    cout << n - ida << endl;
