...
void solve(){
  ...
  cin >> n; cin >> t;
    ...
    for(int i = 0; i < n; i++){
        //if a index is found unmarked, then check all its multiples
        if(t[i]=='0'){
            for(int j = i+1; j<=n; j+=i+1){
                //if multiple is marked, then break cuz u need to leave it
                if(t[j-1]=='1'){
                    break;
                }
                //if already removed by earlier multiple, then continue
                if(s[j-1]=='0'){
                    continue;
                }
                //mark on removing
