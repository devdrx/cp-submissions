...
// ==================================== MATH UTIL ENDS=======================================================//
int gcdOfVector(const std::vector<int>& nums) {
    if (nums.empty()) {
        return 1; // Define what you want the result to be when the vector is empty
    }
    int result = nums[0];
...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        if(abs(i+1-v[i])!=0){
            ans.push_back(abs(i+1-v[i]));
        }
    }
    int uwu = gcdOfVector(ans);
    cout << uwu << endl;
