...
void solve(){
    ...
    cin>>n>>k;
    map<int,vi> mp;
    //store the divisors of every j
    for(int i =1; i<=n; i++){
        for(int j = i; j<=n; j+=i){
            mp[j].push_back(i); //i is pushed back into every number that it divides 
        }
    }
    ...
    int ans = 0;
    //now we have to calculate the sum of all the dp[k][j] for all j
    for(int j = 1; j<=n; j++){
        ans+=dp[k][j];
        ans%=MOD;
    }
