...
void solve(){
    string s; cin >> s;
    int n = s.size();
    //for any segment of 2 characters replace the 2nd one with character not equal to first and after the segment
    for(int i = 0; i < n; i++){
        if(s[i]==s[i+1]){
            for(char c = 'a'; c<='z'; c++){
                if(c!=s[i] && (i==n-1 || c!=s[i+2])){
                    
                    // if c is not equal to s[i] and if c is not equal to s[i+2] if i is not the last index
                    s[i+1]=c;
                    break;
                }
            }
