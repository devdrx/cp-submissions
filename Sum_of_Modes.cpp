...
int mode(string s){
    int maxFreq = 0;
    int freq[2] = {0}; // frequency of 0 and 1

    for (char c : s) {
        freq[c - '0']++;
        maxFreq = max(maxFreq, freq[c - '0']);
    }
...
unordered_multiset<string> printSubstrings(string str)
{
...
void solve(){
    ...
    unordered_multiset<string> substrings = printSubstrings(s);
    int ans = 0;
