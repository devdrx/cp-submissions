...
void solve(){
    ...
    int ans = 0;
    for(auto x: mod){
        if(x.first == 0) ans++; //one array for only zeros
        else if(2*x.first == m) ans++; //one array for m/2 if m even
        //for all ele < m/2 find the corresp pair, if greater than m/2 and not found pair, then it shall be added directly too
        else if(2*x.first < m || mod.find(m-x.first) == mod.end()){
            int o = x.second, p = mod[m-x.first];
            ans+= 1+max(0ll, abs(o-p) - 1);
        }
    }
    cout << ans << endl;
}
