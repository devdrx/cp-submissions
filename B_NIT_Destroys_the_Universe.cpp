...
// ==================================== MATH UTIL ENDS=======================================================//
int countSubsequences(const std::vector<int>& nums) {
    int count = 0;
    ...
    for (int num : nums) {
        if (num == -1) {
            ...
            if (isPrevZero) {
                count++; // Count subsequence of zeros
            }
            if (subsequenceLength > 0) {
                count++; // Count subsequence of -1's followed by 0
            }
            subsequenceLength = 0; // Reset subsequence length
            isPrevZero = true;
        } else {
            isPrevZero = false;
        }
    ...
    // Check if there's a subsequence of -1's at the end of the sequence
    if (subsequenceLength > 0) {
        count++;
    }
...
void solve(){
    ...
    vi v(n);
    cin>>v;
    vi indices; int countNonZeros = 0;
    for(int i = 0; i < n;i++){
        if(v[i]!=0){
            countNonZeros++;
            indices.push_back(i);
    ...
    if(countNonZeros == 0) cout  << 0 <<endl;
    else{
        countNonZeros = 1;
        for(int i = 1; i < indices.size(); i++){
            if(indices[i]- indices[i-1]>1) {countNonZeros = 2; break;}
        }
        cout << countNonZeros << endl;
    }
