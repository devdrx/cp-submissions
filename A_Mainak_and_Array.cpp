...
void solve(){
    ...
    //     }
    // }
    // if(abs(maxindex-minindex)==1||abs(maxindex+minindex==n-1)) cout << v[maxindex]-v[minindex];
    // else if(minindex==0 && maxindex>=1&&maxindex<=n-1){
    //     cout << v[maxindex] - v[minindex];
    // }
    // else if(maxindex==n-1 && minindex>=0 && minindex<=n-2){
    //     cout << v[maxindex] - v[minindex];
    // }
    // else if(maxindex>=1&&maxindex<=n-2&&minindex>=1 && minindex<=n-2){
    //     cout << max(max(max((v[maxindex]-v[0]),(v[n-1]-v[minindex])),max((v[maxindex]-v[maxindex-1]),(v[maxindex]-v[maxindex+1]))),max((v[minindex]-v[minindex-1]),(v[minindex]-v[minindex+1])));
    // }
    // else cout << 0;
    ...
    for(int i = 1; i < n; i++){
        ans2 = max(v[i-1]-v[i], ans2);
    }
    for(int i = 0; i < n-1; i++){
        ans3= max(v[n-1]-v[i], ans3);
    }
    for(int i = 1; i < n; i++){
        ans4 = max(v[i]-v[0], ans4);
    }

    vi answer = {ans1, ans2, ans3, ans4};
    int answ = mxe(answer);
