...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        ...
        a.push_back(b);
        owo.push_back({mxe(b),b.size()});
    }

    int power = 0;
    srt(owo); //sorted caves by increasing order of diff
    int maxi = -1;
    for(int i = 0; i < n; i++){
        owo[i].first -= power; // subtracting power gained from previous caves
        maxi = max(owo[i].first,maxi);
        power += owo[i].second; // adding power gained on this cave
    }
