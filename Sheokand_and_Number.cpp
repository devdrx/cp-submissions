...
void solve(){
    ...
    vector<int> v;
    for (int i = 0; i <= 30; i++) {
        for (int j = i + 1; j <= 30; j++) v.push_back(f[i] + f[j]);
    }
    sort(v.begin(), v.end());
    int idx;
    while (t--) {
        cin >> n;
        idx = lower_bound(v.begin(), v.end(), n) - v.begin(); //this will give the index of the element which is >= n
        if (idx > 1)
            cout << min(abs(v[idx] - n), abs(v[idx - 1] - n)) << endl;
        else
            cout << abs(v[idx] - n) << endl;
    }
