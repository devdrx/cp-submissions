...
void solve(){
    ...
    vi a(n), b(n);
    cin >> a >> b;
    int c1 = INFINITY, c2 = INFINITY, c3 = INFINITY, c4 = INFINITY;
    for(int i = 0; i < n; i++){
    ...
    int ans = c1 + c2 + c3 + c4;
    //cross wire and direct wire
    ans = min(ans, min(abs(a[0]-b[n-1]) + abs(a[n-1]-b[0]), abs(a[0]-b[0])+abs(a[n-1]-b[n-1])));
    //left side fix
    ans = min(ans, abs(a[0]-b[0])+c2+c4);
    //right side fix
    ans = min(ans, abs(a[n-1]-b[n-1])+c1+c3);
    //rtl cross fix
    ans = min(ans, abs(a[n-1]-b[0]) + c1 + c4);
    //ltr cross fix
    ans = min(ans, abs(a[0]-b[n-1]) + c3 + c2);
