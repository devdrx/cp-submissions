...
void solve(){
    ...
    rep(i,1,n){
        if(a[i]<i){
            ans += lower_bound(all(used), a[i]) - used.begin(); //counts no of i's lesser than a[j]'s
            //because i is an increasing sequence, it wont matter if i < j when a[j] < i; no sorting shit reqd (i overthought)
            used.push_back(i);
        }
    }
    cout << ans << endl;
