...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        int num; cin >> num;
        a.push_back(num); //take prefix of i + 1 length of array
        srt(a);           //sort the prefix and arrange the bottles
        int height = 0, end = i;
        while(end>=0){
            height += a[end];
            end-=2;
            if(height > h){ //if at i+1th iteration, the height exceeds h, then i is the most u can take
               cout << i << endl;
               return;
            }
        }
    }
    cout << n << endl; //if height can never exceed, then u just fit all them bottles
