...
void solve(){
    int a, b;
    cin >> a >> b;
    int mov1[4] = {-a, a, -a, a}, mov2[4] = {-b, -b, b, b};
    set<pair<int,int>> st1, st2;
    int xk, yk, xq, yq;
    cin >> xk >> yk >> xq >> yq;
    for(int i = 0; i<4; i++){
        st1.insert(make_pair(xk+mov1[i],yk+mov2[i]));
        st2.insert(make_pair(xq+mov1[i],yq+mov2[i]));
        st1.insert(make_pair(xk+mov2[i],yk+mov1[i]));
        st2.insert(make_pair(xq+mov2[i],yq+mov1[i]));
    }
    int ans = 0;
    for(auto x : st1){
