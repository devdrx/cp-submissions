...
void solve(){
    ...
    string ops;
    cin >> ops;
    //create a n+2, m+2 grid with the boundary filled with #
    char grid2[h+2][m+2];
    for(int i = 0; i < h+2; i++){
        grid2[i][0] = '#';
        grid2[i][m+1] = '#';
    ...
    //initial position is s1,s2
    //simulate the movement operation in ops string, if # is encountered then stay at current pos else move to the coordinate according to the operation U L R D
    int x = s1, y = s2;
