    ...
    for(auto num: mp){
        if(num.second >= k){
            if(count == 0){
                ...
                //old range
                if(left+count==num.first){
                    count++;
                }
                else{
                    //start another range and update the answer
                    if(count > maxi){
                        maxi = count;
                        L = left; //storing the left idx which had the max length
                    }
                    count = 1;
                    left = num.first; //updating left for new range
                }
            }
        }
        else{
