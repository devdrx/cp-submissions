...
void solve(){
    ...
    vi idx1, idx2;
    for(int i = 1; i < n-1; i++){
        if(s1[i]=='.' and s1[i-1]=='x' and s1[i+1]=='x'){
            idx1.push_back(i);
        }
    }
    for(int i = 1; i < n-1; i++){
        if(s2[i]=='.' and s2[i-1]=='x' and s2[i+1]=='x'){
            idx2.push_back(i);
        }
    }
    //for each index in idx1, check if that index and index-1 and index +1 are same and equal to '.'

    int ans = 0;
    for(int i = 0; i < idx1.size(); i++){
        if(s2[idx1[i]] == '.' and s2[idx1[i]-1] == '.' and s2[idx1[i]+1] == '.'){
            ans++;
        }
    }
    //do the same in idx2
    for(int i = 0; i < idx2.size(); i++){
        if(s1[idx2[i]] == '.' and s1[idx2[i]-1] == '.' and s1[idx2[i]+1] == '.'){
            ans++;
        }
    }
