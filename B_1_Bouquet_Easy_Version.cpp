...
int curr = 0; int l = 0;
//we are looping through the end pointer
for(int r = 0; r < n; r++){
    curr += a[r];
    //now i need to check if start and end are valid
    while(petals[r]-petals[l] > 1){
        curr -= a[l]; ///subtract the value as u move start pointer
        l++;
    }
    //now we reached a valid window
    //and we reduce our sum to <m
}
