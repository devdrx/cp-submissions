...
void solve(){
    ...
    for (int i = 1; i < n; ++i) {
        if (a[i] == 1 && a[i - 1] > 1) {
            // If a[i] is 1 and a[i-1] is greater than 1, break the process
        ...
        if (a[i] > 1 && a[i - 1] > 1) {
            double base = a[i];
            double target = a[i - 1];
            // Calculate x = log_a[i](a[i-1])
            double x = log(target) / log(base);

            // Find the smallest power of 2 greater than x
            // int count = 0;
            ...
            // Output the number of squaring operations needed
        }
    }
    cout << count << endl;
}
