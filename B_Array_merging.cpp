...
void solve(){
    ...
    unordered_map<int,int> conseca, consecb;
    int idx=0, val, cnt;
    while(idx<n){
        val = v[idx];
        cnt = 0;
        ...
        conseca[val] = max(conseca[val], cnt);
    }
    idx = 0;
    while(idx<n){
        val = w[idx];
        cnt = 0;
        while(idx < n && w[idx] == val){
            cnt++;
            idx++;
        }
        consecb[val] = max(consecb[val], cnt);
    }
    int ans = 0;
    for(auto it : conseca){
        int vl = it.first;
        int cntatvl_a = it.second;
        int cntatvl_b = consecb[it.first];
        ans = max(cntatvl_a+cntatvl_b, ans);
    }
    for(auto it : consecb){
        int vl = it.first;
        int cntatvl_b = it.second;
        int cntatvl_a = conseca[it.first];
        ans = max(cntatvl_a+cntatvl_b, ans);
    }
    cout << ans << endl;
}
